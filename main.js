...
document.addEventListener('DOMContentLoaded', () => {
    console.log('Winza Store Loaded!');
    loadCartFromStorage();
...
function loadCartFromStorage() {
    const savedCart = localStorage.getItem('winzaCart');
    if (savedCart) {
...
function saveCartToStorage() {
    localStorage.setItem('winzaCart', JSON.stringify(cart));
}

function loadWishlistFromStorage() {
    const savedWishlist = localStorage.getItem('winzaWishlist');
    if (savedWishlist) {
...
function saveWishlistToStorage() {
    localStorage.setItem('winzaWishlist', JSON.stringify(wishlist));
}
...
async function fetchProducts() {
    ...
    try {
        ...
        grid.innerHTML = '<div class="loading"><i class="fa-solid fa-exclamation-circle"></i> Failed to load products. Please refresh the page.</div>';
    }
}
...
function renderProducts(products) {
    ...
    if (productsToShow.length === 0) {
        grid.innerHTML = '<div class="loading"><i class="fa-solid fa-search"></i> No products found</div>';
        return;
    }
    ...
    grid.innerHTML = productsToShow.map(product => `
        <div class="card" data-id="${product.id}">
            <div class="img-box" onclick="openProductModal(${product.id})">
                ...
                <div class="quick-view"><i class="fa-solid fa-eye"></i> Quick View</div>
            </div>
            <div class="info">
...
function updateCart() {
    ...
    // Update items
    if (cart.length === 0) {
        cartItems.innerHTML = '<p class="empty-cart"><i class="fa-solid fa-shopping-bag" style="font-size: 40px; margin-bottom: 15px; display: block;"></i>Your cart is empty</p>';
    } else {
...
function renderWishlist() {
    ...
    if (wishlist.length === 0) {
        wishlistItems.innerHTML = '<p class="empty-wishlist"><i class="fa-regular fa-heart" style="font-size: 40px; margin-bottom: 15px; display: block;"></i>Your wishlist is empty</p>';
        return;
    }
